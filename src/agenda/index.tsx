...
export default class Agenda extends Component<AgendaProps, State> {
  ...
  constructor(props: AgendaProps) {
    ...
    this.state.scrollY.addListener(({value}: any) => this.knobTracker.add(value));
  }
  ...
  onDayPress = (d: DateData = this.state.selectedDay) => {
  ...
  renderWeekDaysNames = memoize((weekDaysNames: string[]) => {
    return weekDaysNames.map((day, index) => (
      <Text key={day + index} style={this.style.weekday} allowFontScaling={false} numberOfLines={1}>
        {day}
      </Text>
    ));
  });
  ...
  render() {
    ...
    return (
      <View testID={testID} onLayout={this.onLayout} style={[style, this.style.container]}>
        ...
        >
          <TouchableOpacity
            onPress={() => this.toggleCalendarPosition(!this.state.calendarScrollable)}
